...
export default function RejectRepayment() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        await Axios({
          ...SummaryApi.reject_reimbursement_by_unique_link,
          params: { token },
        });
        setStatus('success');
      } catch (err: unknown) {
          ...
          <div className="px-8 py-10 text-center">
            ...
            {status === 'success' && (
              <>
                ...
                <h1 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
                  Remboursement rejeté avec succès
                </h1>
                <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed">
                  Vous avez réjeté ce remboursement. L'huissier en charge a été notifié automatiquement.
                </p>
                <div className="mt-6 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl px-4 py-3 text-left">
                  <p className="text-xs text-emerald-700 dark:text-emerald-400 font-medium">
                    ✓ &nbsp;Votre rejet a été enregistré. Vous pouvez fermer cette page.
                  </p>
                </div>
              </>
            )}
